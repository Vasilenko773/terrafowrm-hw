...
resource "null_resource" "web_hosts_provision" {
  ...
  depends_on = [
    ...
    yandex_compute_instance.virtual_db,
    yandex_compute_instance.vm_singleton
  ]

  #Добавление ПРИВАТНОГО ssh ключа в ssh-agent
  provisioner "local-exec" {
    command = "eval $(ssh-agent) && cat ${var.vm_setting["min_performance"].metadata.ssh_key_path} | ssh-add -"
